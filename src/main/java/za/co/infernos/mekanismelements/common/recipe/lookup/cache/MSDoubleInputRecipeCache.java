package za.co.infernos.mekanismelements.common.recipe.lookup.cache;
    ...
    public RECIPE findFirstRecipe(@Nullable Level world, INPUT_A inputA, INPUT_B inputB, boolean useCacheA) {
        if (cacheA.isEmpty(inputA) || cacheB.isEmpty(inputB)) {
            ...
            if (world != null && world.getGameTime() % 100 == 0) {
                 za.co.infernos.mekanismelements.common.MekanismElements.logger.info("DEBUG: findFirstRecipe EMPTY? A: {} | B: {} | RecipeType: {}", cacheA.isEmpty(inputA), cacheB.isEmpty(inputB), recipeType);
            }
            return null;
        }
